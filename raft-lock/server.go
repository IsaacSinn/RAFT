...
func (ls *LockServer) applyToStateMachine(op *Op) {
	// 4C: Your code here
	originalLockOwner, flag := ls.store.getLockOwner(op.Key)

	if op.Type == LockOp {
		ls.store.lock(op.Key, op.ClientId)
		if ls.isLeader() {
			ls.events <- Event{Op: op.Type, key: op.Key, id: op.ClientId, eventType: ServerEvent}
		}
	...
	// If the lock owner has changed, call giveLockToOwner()
	newLockOwner, newFlag := ls.store.getLockOwner(op.Key)

	if newLockOwner != originalLockOwner || (newFlag && !flag) {
